...
const VideoAnnotations = () => {
    const [allVideoAnnotations, setAllVideoAnnotations] = useState<Array<IAnnotation>>(DUMMY_ANNOTATIONS)
    ...
    const [selectionBox, setSelectionBox] = useState({ x: 0, y: 0, width: 0, height: 0 }); // Selection box
    ...
    const calculateSelectedDots = (box) => {
        ...
        return currentAnnotations.flatMap((annotation) =>
            annotation.frames.filter(frame => frame.frame === currentFrame).flatMap((obj) =>
                obj.dots.filter((dot) => {
                    const insideX = dot.x >= normalizedBox.x && dot.x <= normalizedBox.x + normalizedBox.width;
                    const insideY = dot.y >= normalizedBox.y && dot.y <= normalizedBox.y + normalizedBox.height;
    ...
    const handleDotClick = (event, dotId, annotationId) => {
        const clickedPoint = (currentAnnotations.find(ann => ann.id === annotationId) as IAnnotation).frames.find(timestamp => timestamp.frame === currentFrame)?.dots.find(d => d.id === dotId)
    ...
    const currentAnnotations = allVideoAnnotations.filter(ann => ann.frames.some(obj => obj.frame === currentFrame))
    ...
    return (
        <>
            <div style={{ position: 'relative', width: '800px', height: '450px' }} onContextMenu={handleRightClickOnScreen}>
